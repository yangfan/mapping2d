...
bool LikelihoodAlignment::Evaluate(double const *const *params,
  ...
  Sophus::SE2d pose(params[0][2], Eigen::Vector2d(params[0][0], params[0][1]));
  const Eigen::Vector2d query_pos = pose * query_pt_s_;
  const Eigen::Vector2d query_coord =
      likelihood_field_->resolution() * query_pos +
  ...
  *is_outlier_ = outside_map;
  if (outside_map) {
    ...
    residuals[0] =
        likelihood_field_->get_value(query_coord.y(), query_coord.x());
  }
    ...
    if (outside_map) {
      ...
      double grad_y =
          ...
          (likelihood_field_->get_value(query_coord.y() + 1, query_coord.x()) -
           likelihood_field_->get_value(query_coord.y() - 1, query_coord.x()));
      jacobian[0][0] = likelihood_field_->resolution() * grad_x;
      jacobian[0][1] = likelihood_field_->resolution() * grad_y;
      jacobian[0][2] = likelihood_field_->resolution() * range_ *
